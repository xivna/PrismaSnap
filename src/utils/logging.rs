...
//!
//! 双输出：控制台 + 文件（追加写），级别相同。
//! 级别来源：`RUST_LOG` 环境变量优先，否则用设置页配置（改后重启生效）。
...
/// * `level` - 默认过滤级别（`RUST_LOG` 已设置时以环境变量为准）。
///
/// # Errors
...
pub fn init(log_dir: &Path, level: &str) -> anyhow::Result<std::fs::File> {
